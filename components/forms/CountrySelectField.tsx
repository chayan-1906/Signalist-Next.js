				...
				<Button variant={'outline'} role={'combobox'} aria-expanded={open} className={'country-select-trigger'}>
					{value ? (
						<span className={'flex items-center gap-2'}>
                            ...
                            <span>{countries.find((country: Country) => country.value === value)?.label}</span>
						</span>
					) : (
						'Select your country...'
					)}
						...
						<CommandGroup className={'bg-gray-800'}>
							{countries.map((country: Country) => (
								<CommandItem
									...
									<Check className={cn('mr-2 size-4 text-yellow-500', value === country.value ? 'opacity-100' : 'opacity-0')}/>
									<span className='flex items-center gap-2'>
...
function CountrySelectField({name, label, control, error, required}: CountrySelectProps) {
	return (
